...
- The `pyautogui` sends input to the process in focus, which is the `wrap` subprocess.
- If `wrap` is not exited correcly via `q`, it causes weird vertical tabs in print statements from python
- There is some flakiness in sending of input. Many times, `Ctrl+C` is received as `C` in `wrap`
...
- xdotool
  - Seems complicated. It wont be able to manage wrap process that well
...
- pywin32 library
  ...
  - First we need to get it send input to a sample window like notepad, etc. Then we can make wrapper work
- pywinpty
...
- Abstraction layer for wrap init, teardown and inputm
...
if platform.system() == "Windows" :
    ...
    from core.pyautogui_manager import PyAutoGuiWRAPManager
else:
    from core.tmux_manager import TmuxWRAPManager
...
def run_tests(wrap_path : Path, stop_on_fail : bool = True, only_run_tests : List[str] = None) -> bool:
    """Runs tests

    Args:
        wrap_path (Path): Path of wrap binary under test
    ...
    try:
        ...
        wrap_manager : BaseWRAPManager = None
        if platform.system() == "Windows" :
            wrap_manager = PyAutoGuiWRAPManager(str(wrap_path))
        else:
            wrap_manager = TmuxWRAPManager(str(wrap_path))

        test_env = Environment(wrap_manager, fs_manager)

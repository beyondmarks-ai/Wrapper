...
temp_dir=$(mktemp -d)
if [ $? -ne 0 ]; then
    echo -e "${red}❌ Fail install wrapper: ${yellow}Unable to create temporary directory${nc}"
    exit 1
fi

fetch_latest_version() {
    local response
    if response=$(curl -s --max-time 5 "https://api.github.com/repos/beyondmarks/wrapper/releases/latest"); then
        local version
...
package=wrapper
version=${WRAP_INSTALL_VERSION:-$(fetch_latest_version)}
arch=$(uname -m)
os=$(uname -s)
...
if [[ "$arch" == "x86_64" || "$arch" == "amd64" ]]; then
    ...
    echo -e "${red}❌ Fail install wrapper: ${yellow}Unsupported architecture${nc}"
    exit 1
fi
...
if [[ "$os" == "Linux" ]]; then
    ...
    echo -e "${red}❌ Fail install wrapper: ${yellow}Unsupported operating system${nc}"
    exit 1
fi

file_name=${package}-${os}-v${version}-${arch}

url="https://github.com/beyondmarks-ai/Wrapper/releases/download/v${version}/${file_name}.tar.gz"
...
if ! sudo mv ./wrap /usr/local/bin/; then
  echo -e "${yellow}Unable to move binary to /usr/local/bin. Do you have sudo permissions?${nc}"
  mkdir -p ~/.local/bin
  if ! mv ./wrap ~/.local/bin/; then
    echo -e "${red}❌ Failed to install wrapper: Unable to move to ~/.local/bin as well.${nc}"
  else
    ...
    echo -e "${bright_cyan}You can type ${white}\"${bright_yellow}wrap${white}\" ${bright_cyan}to start!${nc}"
  fi
else

...
try {
    if (Test-Path $wrapperProgramPath) {
        Remove-Item -Path $wrapperProgramPath -Recurse -Force
    }
}
catch {
...
try {
    if (FolderIsInPATH "$wrapperProgramPath\") {
        $envPath = [Environment]::GetEnvironmentVariable("PATH", "User")
        $updatedPath =($envPath.Split(';') | Where-Object { $_ -ne "$wrapperProgramPath" }) -join ';'

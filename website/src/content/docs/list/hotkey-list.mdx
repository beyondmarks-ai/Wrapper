---
title: Hotkey list
description: wrapper hotkey list
head:
  - tag: title
    content: Hotkey list | wrapper
---
...
| Quit typing, modal or wrapper         | `q`, `esc`            | `quit`              |
| Quit wrapper and cd to current folder | `Q`                   | `cd_quit`           |
...
| Open prompt in wrap mode                 | `>`                   | `open_wrap_prompt`   |
| Open zoxide navigation modal            | `z`                   | `open_zoxide`       |
| Search Everything index                  | `ctrl+g`              | `open_everything`   |

:::note

Quit wrapper and cd to current folder "cd_quit" requires the same setup as ["cd_on_quit"](/configure/wrapper-config/#cd_on_quit) setting

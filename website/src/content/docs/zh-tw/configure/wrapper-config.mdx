---
title: wrapper 設定
description: 設定你的 wrapper
head:
  - tag: title
    content: wrapper 設定 | wrapper
---

import CodeBlock from "../../../../components/code.astro";

你可以使用以下指令編輯 wrapper 設定檔：
...
若要查看 wrapper 檔案的路徑位置，請使用 `wrap pl` 指令。
...
`true` => 離開 wrapper 時檢查是否需要更新（每天只檢查一次）。

`false` => 不執行檢查。

- ###### cd_on_quit

`true` => 離開 wrapper 時，將終端機路徑變更為你最後使用的 file 面板。

`false` => 離開 wrapper 時，終端機路徑維持進入 wrapper 前的位置。
...
`true` => 執行 wrapper 時顯示檔案預覽視窗。

`false` => 執行 wrapper 時隱藏檔案預覽視窗。

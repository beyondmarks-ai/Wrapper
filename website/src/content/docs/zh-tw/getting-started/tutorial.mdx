---
title: 學習如何使用教學
description: 快速開始使用 wrapper
head:
  - tag: title
    content: 教學 | wrapper
---

本教學會一步一步教你如何使用 wrapper。

:::caution

如果你還沒有安裝 wrapper，請[點這裡](/zh-tw/getting-started/installation)。
...
先啟動 wrapper 吧！開啟終端機，輸入 `wrap` 並按下 `enter`。
...
wrapper 提供多種快捷鍵讓你在目錄間移動。角括號游標 `>` 會指出你目前所在的位置。
...
按 `/` 叫出搜尋列。輸入名稱（如果 `/` 自動帶入，你可能需要先刪掉它）。wrapper 會在目前目錄中搜尋並動態顯示結果。若要離開搜尋列，請按 `ctrl`+`c` 或 `esc`。
...
這些是設定檔中的變更。更多資訊請參閱 [wrapper-config](/zh-tw/configure/wrapper-config)。這會將 `nano` 設為你的預設編輯器，並將 `vi` 設為你的預設目錄編輯器。設定後，使用 `e` 按鍵綁定開啟檔案時會使用 `nano`，使用 `E` 按鍵綁定開啟目前目錄時會使用 `vi`。
...
![Prompt-WRAP-Mode](../../../../assets/tutorial/prompt_wrap_mode.png)

在這個模式中，你可以執行這些 wrap 指令：

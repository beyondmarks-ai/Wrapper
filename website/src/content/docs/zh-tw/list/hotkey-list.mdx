---
title: 快捷鍵清單
description: wrapper 快捷鍵清單
head:
  - tag: title
    content: 快捷鍵清單 | wrapper
---
...
| 離開輸入、視窗或 wrapper        | `q`, `esc`            | `quit`              |
| 離開 wrapper 並 cd 到目前資料夾 | `Q`                   | `cd_quit`           |
| 確認輸入內容                      | `enter`               | `confirm_typing`    |
| 取消輸入                          | `ctrl+c`, `esc`       | `cancel_typing`     |
| 開啟說明選單（快捷鍵清單）        | `?`                   | `open_help_menu`    |
| 以 shell 模式開啟提示列           | `:`                   | `open_command_line` |
| 以 wrap 模式開啟提示列             | `>`                   | `open_wrap_prompt`   |
| 開啟 zoxide 導覽視窗              | `z`                   | `open_zoxide`       |

:::note

離開 wrapper 並 cd 到目前資料夾的 `cd_quit` 需要與 [`cd_on_quit`](/zh-tw/configure/wrapper-config/#cd_on_quit) 設定相同的腳本。

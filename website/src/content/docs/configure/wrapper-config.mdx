---
title: wrapper config
description: Configure your wrapper
head:
  - tag: title
    content: wrapper config | wrapper
---

import CodeBlock from "../../../components/code.astro";

You can edit your wrapper config file with the following command:
...
To see the path locations of your wrapper files, use the command `wrap pl`.
...
`true` => Checks whether updates are needed when you exit wrapper (only checks once a day).

`false` => No checks performed.

- ###### cd_on_quit

`true` => When you exit wrapper, changes the terminal path to the last file panel you used.

`false` => When you exit wrapper, the terminal path remains the same prior to wrapper.
...
`true` => Shows the file preview window when you run wrapper.

`false` => Hides the file preview window when you run a wrapper.
...
The default location every time wrapper is opened. Supports `~` and `.`
...
<CodeBlock file="src/wrapper_config/config.toml" />

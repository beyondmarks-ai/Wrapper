---
title: Image Preview
description: Learn how image preview works in wrapper and how terminal compatibility is determined.
head:
  - tag: title
    content: Image Preview | wrapper
---

This tutorial will teach you how to use wrapper’s image preview feature step by step.

## What is Image Preview?

wrapper supports image previews directly in your terminal using several display protocols. When supported, images can be shown inline without any external viewer.

---

## Terminal Compatibility

wrapper automatically detects your terminal using the `$TERM` and `$TERM_PROGRAM` environment variables. We support rendering on the following terminals:
...
wrapper supports the following rendering protocols and will automatically choose the best one based on your terminal:
...
wrapper detects terminal capabilities by inspecting:
...
To scale images correctly, wrapper sends the following escape code:

```
\x1b[16t
```

This sequence queries the terminal for the size of each **cell in pixels**. wrapper uses the result to:
...
When advanced image preview isn't supported (for example, when the terminal doesn't support the Kitty protocol), wrapper gracefully falls back to an ANSI-based preview using color-coded blocks.

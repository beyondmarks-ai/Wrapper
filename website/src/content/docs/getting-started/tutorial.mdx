---
title: Tutorial
description: Quickly get started with wrapper
head:
  - tag: title
    content: Tutorial | wrapper
---

This tutorial will teach you how to use wrapper step by step.

:::caution

If you haven't installed wrapper yet, please [click here](/getting-started/installation).
...
Let's start by running wrapper! Open a terminal, type `wrap` and press `enter`.
...
Once wrapper is running, it displays five panels:
...
wrapper provides multiple hotkeys to move through directories. The angle bracket cursor `>` tells you where you are.
...
Press `/` to bring up the search bar. Type the name (you may need to first delete the `/` if it auto-populates). wrapper searches in the current directory and dynamically displays the results. To exit the search bar, press `ctrl`+`c` or `esc`.
...
Press `>` to open the prompt in WRAP mode. ![Prompt-WRAP-Mode](../../../assets/tutorial/prompt_wrap_mode.png)

In this mode, you can execute these wrap commands :

...
func TestMain(m *testing.M) {
	...
	// Create testDir
	testDir = filepath.Join(os.TempDir(), "wrap_testdir")
	cleanupTestDir()
...
func TestQuit(t *testing.T) {
	...
	t.Run("Cd on quit test that LastDir is written on", func(t *testing.T) {
		lastDirFile := filepath.Join(variable.WrapperStateDir, "lastdir")
...
func prepareLockUnlockTest(t *testing.T) *model {
	...
	m.wrapError = wraperror.New(true,
		"error",
		"ERROR TEST", wraperror.NewFileListError([]string{file1}, processor, finalizer))
	m.wrapError.Open()
	return m
}

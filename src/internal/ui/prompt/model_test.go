...
func defaultTestModel() Model {
	return GenerateModel(wrapPromptChar, shellPromptChar, true, defaultTestMaxHeight, defaultTestWidth)
}
...
func TestModel_HandleUpdate(t *testing.T) {
	...
	t.Run("Pressing confirm on empty input", func(t *testing.T) {
		actualTest := func(closeOnSuccess bool, openAfterEnter bool) {
			m := GenerateModel(wrapPromptChar, shellPromptChar, closeOnSuccess, defaultTestMaxHeight, defaultTestWidth)
			m.Open(true)
	...
	t.Run("Switching between shell and WRAP mode", func(t *testing.T) {
...
func TestModel_HandleResults(t *testing.T) {
	...
	t.Run("Verify WRAP results update", func(t *testing.T) {
		m := GenerateModel(wrapPromptChar, shellPromptChar, false, defaultTestMaxHeight, defaultTestWidth)
		m.Open(true)
		msg := "Test message"
		m.HandleWRAPActionResults(true, msg)
		...
		// Validate close happens when closeOnSuccess is true
		m.HandleWRAPActionResults(true, "")
		assert.False(t, m.IsOpen())
	})
}
...
func TestModel_Render(t *testing.T) {
	// Test
	// 1 - Default view with shell mode and wrap prompt mode
	// 2 - User input
	...
	t.Run("Basic Render Checks", func(t *testing.T) {
		m := GenerateModel(wrapPromptChar, shellPromptChar, true, 10, 40)
		m.setShellMode(true)
		res := ansi.Strip(m.Render())
		exp := "" +
			"╭─┤ " + icon.Terminal + " wrapper Prompt (Shell Mode) ├────╮\n" +
		...
		res = ansi.Strip(m.Render())
		exp = "" +
			"╭─┤ " + icon.Terminal + " wrapper Prompt (WRAP Mode) ├─────╮\n" +
			// 23--------4------------56789012345678901234567890123456789
			"│ >                                    │\n" +
			"├──────────────────────────────────────┤\n" +
			"│ ':' - Get into Shell mode            │\n" +
	...
	t.Run("Test User Input", func(t *testing.T) {
		execute := func(input string, expected string) {
			...
			width := 10
			m := GenerateModel(wrapPromptChar, shellPromptChar, true, 10, width)
			m.Open(true)
	...
	t.Run("Result Message", func(t *testing.T) {
		m := GenerateModel(wrapPromptChar, shellPromptChar, true, 10, 50)
		m.setShellMode(true)
		m.HandleShellCommandResults(0, "")
		res := ansi.Strip(m.Render())
		exp := "" +

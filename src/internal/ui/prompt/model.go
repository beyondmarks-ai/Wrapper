...
func DefaultModel(maxHeight int, width int) Model {
	return GenerateModel(common.Hotkeys.OpenWRAPPrompt[0],
		common.Hotkeys.OpenCommandLine[0], common.Config.ShellCloseOnSuccess, maxHeight, width)
}

func GenerateModel(wrapPromptHotkey string, shellPromptHotkey string, closeOnSuccess bool,
	...
	m := Model{
		...
		wrapPromptHotkey:  wrapPromptHotkey,
...
func (m *Model) handleNormalKeyInput(msg tea.KeyPressMsg) tea.Cmd {
	var cmd tea.Cmd
	switch {
	case m.textInput.Value() == "" && msg.String() == m.wrapPromptHotkey:
...
func (m *Model) HandleWRAPActionResults(success bool, msg string) {
...
func (m *Model) Render() string {
	...
	if !m.shellMode {
		...
		r.AddSection()
		r.AddLines(" '" + m.wrapPromptHotkey + "' - Get into WRAP mode")
	}

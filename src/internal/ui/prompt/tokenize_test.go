...
const (
	wrapTestEnvVar1 = "WRAP_TEST_ENV_VAR1"
	wrapTestEnvVar2 = "WRAP_TEST_ENV_VAR2"
	wrapTestEnvVar3 = "WRAP_TEST_ENV_VAR3"
	wrapTestEnvVar4 = "WRAP_TEST_ENV_VAR4"
)

var testEnvValues = map[string]string{ //nolint:gochecknoglobals // This is more like a const.
	wrapTestEnvVar1: "1",
	wrapTestEnvVar2: "hello",
	wrapTestEnvVar3: "",
}
...
func Test_resolveShellSubstitution(t *testing.T) {
	...
	testdata := []struct {
		...
		},
		{
			...
			command:         fmt.Sprintf("$(echo $(echo hi)) ${%s}", wrapTestEnvVar2),
			expectedResult:  fmt.Sprintf("hi%s %s", newLineSuffix, testEnvValues[wrapTestEnvVar2]),
			isErrorExpected: false,
			errorToMatch:    nil,
		},
		{
			name:            "Non Existing env var",
			command:         fmt.Sprintf("${%s}", wrapTestEnvVar4),
			expectedResult:  "",
			isErrorExpected: true,
			errorToMatch:    envVarNotFoundError{varName: wrapTestEnvVar4},
		},
		{
	...
	t.Run("Testing shell substitution timeout", func(t *testing.T) {
		result, err := resolveShellSubstitution(100*time.Millisecond, "$(sleep 2)", defaultTestCwd)

...
// RevealFile opens the file's parent directory and positions the cursor on it.
func (m *Model) RevealFile(path string) error {
	parent := filepath.Dir(path)
	name := filepath.Base(path)
	m.SearchBar.SetValue("")

	if parent == m.Location {
		idx := m.FindElementIndexByName(name)
		if idx != -1 {
	...
	if err := m.UpdateCurrentFilePanelDir(parent); err != nil {
		return err
	}
	m.TargetFile = name
	return nil

...
import (
	...
	stringfunction "github.com/beyondmarks-ai/Wrapper/src/pkg/string_function"
)
...
func InitialModel(firstPanelPaths []string, firstUseCheck bool) tea.Model {
	...
	m := defaultModelConfig(toggleDotFile, toggleFooter, firstUseCheck, firstPanelPaths, zClient)
	m.welcomeOpen = os.Getenv("WRAPPER_SKIP_WELCOME") != "1"
	return m
}
...
func (m *model) Update(msg tea.Msg) (tea.Model, tea.Cmd) {
	...
	if m.welcomeOpen {
		...
		case tea.KeyPressMsg:
			return m, m.handleWelcomeKey(msg)
		case tea.MouseMsg:
			return m, nil
		}
	}
	...
	case everythingui.QueryMsg:
		updateCmd = msg.Apply(&m.everythingModal)
	case everythingui.UpdateMsg:
		updateCmd = msg.Apply(&m.everythingModal)
	case everythingui.DevicesUpdateMsg:
		updateCmd = msg.Apply(&m.everythingModal)
...
func (m *model) handleKeyInput(msg tea.KeyPressMsg) tea.Cmd {
	...
	switch {
	case m.wrapError.IsOpen():
		cmd = m.wrapErrorModelOpenKey(msg.String())
...
func (m *model) applyPromptModalAction(action common.ModelAction) tea.Cmd {
	...
	if actionErr != nil {
		m.promptModal.HandleWRAPActionResults(false, actionErr.Error())
	} else if successMsg != "" {
		m.promptModal.HandleWRAPActionResults(true, successMsg)
	}
	return cmd
}
...
func (m *model) logAndExecuteAction(action common.ModelAction) (string, tea.Cmd, error) {
	...
	case common.RevealPathAction:
		if action.IsDir {
			return "Panel directory changed", nil, m.updateCurrentFilePanelDir(action.Path)
		}
		return "File revealed", nil, m.getFocusedFilePanel().RevealFile(action.Path)
...
func (m *model) warnModalForQuit() {
	m.notifyModel = notify.New(true, "Confirm to quit wrapper",
...
func (m *model) updateRenderForOverlay(finalRender string) string {
	...
	if m.wrapError.IsOpen() {
		errorModal := m.wrapError.Render()
	...
	if m.everythingModal.IsOpen() {
		...
		overlayY := m.fullHeight/common.CenterDivisor - m.everythingModal.GetMaxHeight()/common.CenterDivisor
		return stringfunction.PlaceOverlay(overlayX, overlayY, everythingModal, finalRender)
	}
...
// Close wrapper application. Cd into the current dir if CdOnQuit on and save
// the path in state direcotory
func (m *model) quitWrapper(cdOnQuit bool) {
	// Resource cleanup
	...
	slog.Debug("Quitting wrapper", "current dir", currentDir)
}
...
func (m *model) setEverythingModelSize() {
	m.everythingModal.SetMaxHeight(m.fullHeight / 2) //nolint:mnd // modal uses half height for layout
	m.everythingModal.SetWidth(m.fullWidth * 3 / 4)  //nolint:mnd // paths benefit from a wider modal
}

func (m *model) applyEverythingModalAction(action common.ModelAction) tea.Cmd {
	switch action.(type) {
	case common.RemoteTransferAction, common.SendTransferAction:
		return m.remoteTransferCmd(action)
	}
	_, cmd, err := m.logAndExecuteAction(action)
	if err != nil {
		slog.Error("Could not reveal Everything result", "error", err)
		m.notifyModel = notify.New(true, "Unable to open search result", err.Error(), notify.NoAction)
	}
	return cmd
}

...
func TestFileCreation(t *testing.T) {
	...
	for _, tt := range testdata {
		t.Run(tt.name, func(t *testing.T) {
			...
			if tt.expectedError {
				require.Eventually(t, func() bool {
					return m.wrapError.IsOpen()
				}, DefaultTestTimeout, DefaultTestTick, "WRAP error modal never opened for input: %q", tt.fileName)

				p.SendKey(common.Hotkeys.Quit[0])
				require.Eventually(t, func() bool {
					return !m.wrapError.IsOpen()
				}, DefaultTestTimeout, DefaultTestTick, "WRAP error modal never closed")
				return
			}
			...
			assert.False(t, m.wrapError.IsOpen(), "Unexpected WRAP error for input: %q", tt.fileName)
		})
	}

...
type ConfigType struct {
	Theme string `toml:"theme" comment:"More details are at https://wrapper.dev/configure/wrapper-config/\nchange your theme"`
	...
	CdOnQuit               bool   `toml:"cd_on_quit"                comment:"\nCd on quit (For more details, please check out https://wrapper.dev/configure/wrapper-config/#cd_on_quit)"`
	DefaultOpenFilePreview bool   `toml:"default_open_file_preview" comment:"\nWhether to open file preview automatically every time wrapper is opened."`
	...
	DefaultDirectory       string `toml:"default_directory"         comment:"\nThe path of the first file panel when wrapper is opened."`
...
type HotkeysType struct {
	...
	OpenWRAPPrompt  []string `toml:"open_wrap_prompt"`
	OpenZoxide      []string `toml:"open_zoxide"`
	OpenEverything  []string `toml:"open_everything"`
	TransferItems   []string `toml:"transfer_items"`

...
import (
	...
	"github.com/beyondmarks-ai/Wrapper/src/internal/trash"
)
	...
	if err != nil {
		...
		if errors.As(err, &loadError) && loadError != nil {
			if loadError.MissingFields() && !variable.FixConfigFile {
				...
				userMsg += "\nTo add missing fields to configuration file automatically run wrapper " +
					"with the --fix-config-file flag `wrap --fix-config-file`"
			}
	...
	if err != nil {
		...
		if errors.As(err, &loadError) {
			if loadError.MissingFields() && !variable.FixHotkeys {
				// Had missing fields and we did not fix
				userMsg += "\nTo add missing fields to hotkeys file automatically run wrapper " +
					"with the --fix-hotkeys flag `wrap --fix-hotkeys`"
			}
			toExit = loadError.IsFatal()
		}
		if toExit {
...
// LoadAllDefaultConfig : Load all default configurations from embedded wrapper_config folder into global
...
func PopulateGlobalConfigs() error {
	...
	// we want src/wrapper_config
	wrapConfigDir := filepath.Join(filepath.Dir(filepath.Dir(filepath.Dir(filename))),
		"wrapper_config")

	configFilePath := filepath.Join(wrapConfigDir, "config.toml")
	hotkeyFilePath := filepath.Join(wrapConfigDir, "hotkeys.toml")
	themeFilePath := filepath.Join(wrapConfigDir, "theme", "monokai.toml")

...
type WrapErrorModalUpdateMsg struct {
	BaseMessage

	m wraperror.Model
}

func NewWrapErrorModalMsg(m wraperror.Model, reqID int) WrapErrorModalUpdateMsg {
	return WrapErrorModalUpdateMsg{
		m: m,
		BaseMessage: BaseMessage{
			reqID: reqID,
		},
...
func (msg WrapErrorModalUpdateMsg) ApplyToModel(m *model) tea.Cmd {
	m.wrapError = msg.m
	return nil
}

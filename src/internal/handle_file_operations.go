...
func (m *model) runFileProcessor(processor processbar.FileListProcessor,
	...
	if result.State == processbar.Failed && len(toDo) > 0 {
		// we unlock mutexErrorModal on dispatch WrapErrorModalUpdateMsg
		errorModel := wraperror.New(true,
			"Error",
			result.ErrorMsg, wraperror.NewFileListError(toDo, processor, finalizer))
		return NewWrapErrorModalMsg(errorModel, reqID)
	}
...
// Copy directory or file's path to wrapper's clipboard

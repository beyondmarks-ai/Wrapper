...
func (m *model) mainKey(msg string) tea.Cmd { //nolint: gocyclo,cyclop,funlen,gocognit // See above
	...
	case slices.Contains(common.Hotkeys.OpenWRAPPrompt, msg):
		m.promptModal.Open(false)
	case slices.Contains(common.Hotkeys.OpenZoxide, msg):
		return m.zoxideModal.Open()
	case slices.Contains(common.Hotkeys.OpenEverything, msg):
		return m.everythingModal.Open()
	case slices.Contains(common.Hotkeys.TransferItems, msg):
		return m.openRemoteSendPicker()
...
func (m *model) wrapErrorModelOpenKey(msg string) tea.Cmd {
	isAbort := slices.Contains(wraperror.KeyAbort(), msg)
	isSkip := slices.Contains(wraperror.KeySkip(), msg)

	if !isAbort && !isSkip {
		slog.Warn("Invalid keypress in wrapErrorModel", "msg", msg)
		return nil
	}
	defer func() {

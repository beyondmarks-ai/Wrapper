...
import (
	...
	internal "github.com/beyondmarks-ai/Wrapper/src/internal"
)
	...
	app := &cli.Command{
		...
		Commands: []*cli.Command{
			{
				...
				Action: func(_ context.Context, c *cli.Command) error {
					...
					fmt.Printf("%-*s %s\n", common.HelpKeyColumnWidth,
						configStyle.Render("[Configuration directory path]"), variable.WrapperMainDir)
					fmt.Printf("%-*s %s\n", common.HelpKeyColumnWidth,
						dataStyle.Render("[Data directory path]"), variable.WrapperDataDir)
					return nil
				},
				Flags: []cli.Flag{
					&cli.BoolFlag{
		...
		Flags: []cli.Flag{
			...
			&cli.StringFlag{
				Name:    "chooser-file",
				Aliases: []string{"cf"},
				Usage:   "On trying to open any file, wrapper will write to its path to this file, and exit",
				Value:   "", // Default to the blank string indicating non-usage of flag
			},
		},
		Action: wrapAppAction,
	}
...
func wrapAppAction(_ context.Context, c *cli.Command) error {
	...
	// If no args are called along with "wrap" use current dir
...
func InitConfigFile() {
	...
	if err := utils.CreateDirectories(
		variable.WrapperMainDir,
		variable.WrapperDataDir,
		variable.WrapperStateDir,

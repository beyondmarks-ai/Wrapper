...
usage() {
    ...
    echo ""
    echo "A comprehensive script for formatting, testing, and building wrapper"
    echo ""
    echo "OPTIONS:"
...
printf "${BLUE}🚀 Starting wrapper development workflow${NC}\n"
echo ""
...
print_step "Building wrap binary..."
if [ "$(go env GOOS)" = "darwin" ]; then
    BUILD_CMD=(env CGO_ENABLED=1 go build -o ./bin/wrap)
else
    BUILD_CMD=(env CGO_ENABLED=0 go build -o ./bin/wrap)
fi
if "${BUILD_CMD[@]}"; then
...
printf "${BLUE}Binary location:${NC} ./bin/wrap\n"

# Show binary info
if [ -f "./bin/wrap" ]; then
    BINARY_SIZE=$(du -h ./bin/wrap | cut -f1)
